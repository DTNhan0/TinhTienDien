...
public class FilterLoaiDateTTGDDialog extends javax.swing.JDialog {
    private MainAdminView MainAdminview;
    private ThongTinGhiDienMainView thongTinGhiDienMainView;
    
    public FilterLoaiDateTTGDDialog(MainAdminView mainAdminView, ThongTinGhiDienMainView ttgdmv, boolean modal) {
        initComponents();
    ...
    private void OkBtActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_OkBtActionPerformed
        ...
        if(NgayCkb.isSelected()){
            DSNhanVien.Filter((String)NgayCb.getSelectedItem(), 1, 1, thongTinGhiDienMainView.getBangDSNhanVien());
            check = true;
        }
        if(ThangCkb.isSelected()){
            DSNhanVien.Filter((String)ThangCb.getSelectedItem(), 2, 1, thongTinGhiDienMainView.getBangDSNhanVien());
            check = true;
        }
        if(NamCkb.isSelected()){
            DSNhanVien.Filter(String.valueOf(NamCb.getYear()), 3, 1, thongTinGhiDienMainView.getBangDSNhanVien()); 
            check = true;
        }
        if(check)
