...
public class FilterLoaiDateTTNXDialog extends javax.swing.JDialog {
    private MainAdminView MainAdminview;
    private ThongTinNhapXuatMainView thongTinNhapXuatMainView;
    
    public FilterLoaiDateTTNXDialog(MainAdminView mainAdminView, ThongTinNhapXuatMainView ttnxmv, boolean modal) {
        initComponents();
    ...
    private void OkBtActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_OkBtActionPerformed
        ...
        if(NgayCkb.isSelected()){
            DSNhanVien.Filter((String)NgayCb.getSelectedItem(), 1, 1, thongTinNhapXuatMainView.getBangDSNhanVien());
            check = true;
        }
        if(ThangCkb.isSelected()){
            DSNhanVien.Filter((String)ThangCb.getSelectedItem(), 2, 1, thongTinNhapXuatMainView.getBangDSNhanVien());
            check = true;
        }
        if(NamCkb.isSelected()){
            DSNhanVien.Filter(String.valueOf(NamCb.getYear()), 3, 1, thongTinNhapXuatMainView.getBangDSNhanVien()); 
            check = true;
        }
        if(check)
